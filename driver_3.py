...
if __name__ == "__main__":
    ...
    def generate_neighbors(n_puzzle_state, distance=None):
        ...
        for move, row_delta, col_delta in moves:
            ...
            # New path:
            new_path_to_state = n_puzzle_state.path_to_state + str(move)
            # New path cost:
    ...
    def generate_report(goal_state, goal_stats):
        ...
        report = "\n".join(
            [
                "path_to_goal: {}".format(list(moves[int(idx)] for idx in goal_state.path_to_state) if not goal_state is None else None),
                "cost_of_path: {}".format(goal_state.cost_of_path if not goal_state is None else None),
                "nodes_expanded: {}".format(goal_stats['nodes_expanded'] - 1),
                "fringe_size: {}".format(goal_stats['fringe_size']),
                "max_fringe_size: {}".format(goal_stats['max_fringe_size']),
                "search_depth: {}".format(goal_state.search_depth if not goal_state is None else None),
                "max_search_depth: {}".format(goal_stats['max_search_depth']),
                "running_time: {:.8f}".format(goal_stats['running_time']),
                "max_ram_usage: {:.8f}".format(goal_stats['max_ram_usage'])
            ]
        )
        return report
        ...
        def do_ida_star_solve(root, bound, stats):
            ...
            while not frontier.is_empty():
                ...
                if state.heuristic_cost > bound:
                    min_cost = 2*state.heuristic_cost
                    break
